...
expense_trend = expense_data.groupby(["Month", "Category"])["Amount"].sum().reset_index()
fig_expense_trend = px.line(expense_trend, x="Month", y="Amount", color="Category", markers=True, title="Expense Trend by Category")
...
if st.button("📄 Generate Financial Report (PDF)"):
    pdf_generator.create_pdf_report(filtered_df, pdf_filename)
